...
RUST_ROOT :=$(abspath kernel/noxrust)
RUST_BUILD :=$(abspath kernel/noxrust/target/x86-nox/debug)

# Find all candidates for compiling
ASOURCE := $(shell find $(ASOURCE_DIR) -name '*.asm')
CSOURCE := $(shell find $(CSOURCE_DIR) -name '*.c')
RSSOURCE :=$(shell find $(RUST_ROOT)/src -name '*.rs')
...
CLEAN_DIRS += $(OBJ_DIR) $(DEP_DIR) $(RUST_ROOT)/target
...
$(BUILD_DIR)/kernel.elf: $(COBJECTS) $(AOBJECTS) $(KERNEL_LINKER_SCRIPT) $(RUST_BUILD)/libnoxrust.a
	@echo "$(TIME) LD       $@"
	$(TOOL)-ld -T $(KERNEL_LINKER_SCRIPT) -Map=$(BUILD_DIR)/kernel.map $(filter-out $(KERNEL_LINKER_SCRIPT), $^) -o $@ -L$(RUST_DIR) -lnoxrust

$(RUST_BUILD)/libnoxrust.a: $(RSSOURCE)
	cd $(RUST_ROOT) && cargo xbuild
